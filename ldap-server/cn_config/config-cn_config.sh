...
/bin/bash encrease-max_for_queries_in_DOMAIN_database.sh

if [ "$LDAP_IS_SLAVE_SERVER" = "yes" ];
then
    printAndLogMessage "add replication for DOMAIN database to slave cn=schema,cn=config"
    /bin/bash add-replication_for_DOMAIN_database_to_slave_cn_config.sh

    printAndLogMessage "disable write to DOMAIN database for slave"
    /bin/bash disable-write_to_DOMAIN_database_for_slave.sh
fi
